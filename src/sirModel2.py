...
class SIRmodel(object):
	...
	def init_graph(self,g='reg',k=4):
		...
		gtype = { 'er':nx.fast_gnp_random_graph(self.population,0.01),
	...
	def run_sim(self, length=100, graphtype = None,start=0, inc=1 ):
		"""Run our simulation.
		...
		#failed to randomly add individual doing it anyway
		if len(iList) == 0:
			pzero = random.choice(sList)
			pzero.infectAgent()
			iList.append(pzero)
			sList.remove(pzero)
		...
		while(len(iList) != 0 and start < self.days):
			...
			self.susceptibles[start] = (len(sList)/float(self.population))
			self.infected[start] = (len(iList)/float(self.population))
			self.recoverers[start] = (len(rList)/float(self.population))
	...
	def plot_graphs(self,fig, timep):
		...
		if self.graphtype == 'grid':
			position = dict(zip(self.diseasenetwork,self.diseasenetwork))
		else:
	...
	def plot_sim(self):
		...
		#pl.subplot(211)
		pl.plot(self.susceptibles, '-g', label='Susceptibles')
		pl.plot(self.recoverers, '-k', label='Recovered')
		pl.plot(self.infected, '-r', label='Infectious')
		pl.legend(loc=0)
...
def __main__():
	...
	infect = [] 
	graphtypes =  [ 'er','nws','cws','ba','grid','reg']
	...
	pl.show()
		...
		#model.plot_sim()
		#print "For model {}    cluster {} ".format(model.name,cluster)
		#fig = pylab.figure(figsize=(10,10),dpi=80)
		#model.plot_graphs(fig,"end")
		#print avgform
	
	#TODO: plot each type of infected on same map
